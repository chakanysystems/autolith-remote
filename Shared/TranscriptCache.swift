...
struct CachedTranscript: Codable, Sendable {
    ...
    enum CacheError: Error { case invalidRevision }
}
...
actor TranscriptCache {
    ...
    func save(_ snapshot: Snapshot, key: String) throws {
        ...
        // A credential change must not leave old authenticated cache files accumulating.
        for old in (try? FileManager.default.contentsOfDirectory(at: directory, includingPropertiesForKeys: nil)) ?? [] where old.standardizedFileURL.path != url.standardizedFileURL.path {
