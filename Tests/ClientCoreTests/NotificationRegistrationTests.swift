...
final class NotificationRegistrationTests: XCTestCase {
    ...
    func testRevocationRejectsInflightAcceptance() throws {
        ...
        let request = try XCTUnwrap(state.beginRemoteRegistration(host: "computer", now: start))
        XCTAssertEqual(state.revoke(host: "computer"), "token")
        XCTAssertFalse(state.finishRemoteRegistration(request, accepted: true, now: start))
        XCTAssertFalse(state.usesRemoteNotifications(host: "computer", now: start))
        XCTAssertNil(state.beginRemoteRegistration(host: "computer", now: start))
    }

    func testCredentialAndGenerationChangesInvalidateAcceptedAndPendingRegistration() throws {
        var state = NotificationRegistration()
        state.receivedDeviceToken("apns")
        let first = ConnectionContext(host: "computer", token: "credential-a", generation: 0)
        state.activate(context: first)
        let accepted = try XCTUnwrap(state.beginRemoteRegistration(host: "computer", now: start))
        XCTAssertTrue(state.finishRemoteRegistration(accepted, accepted: true, now: start))
        state.activate(context: ConnectionContext(host: "computer", token: "credential-b", generation: 0))
        XCTAssertFalse(state.usesRemoteNotifications(host: "computer", now: start))
        let pending = try XCTUnwrap(state.beginRemoteRegistration(host: "computer", now: start))
        state.activate(context: ConnectionContext(host: "computer", token: "credential-b", generation: 1))
        XCTAssertFalse(state.finishRemoteRegistration(pending, accepted: true, now: start))
        XCTAssertNotNil(state.beginRemoteRegistration(host: "computer", now: start))
    }

    func testRevokedContextCannotReregisterUntilSwitchCompletes() throws {
        var state = NotificationRegistration()
        state.receivedDeviceToken("apns")
        let first = ConnectionContext(host: "computer", token: "credential", generation: 0)
        state.activate(context: first)
        _ = state.beginRemoteRegistration(host: "computer", now: start)
        _ = state.revoke(host: "computer")
        state.activate(context: first)
        state.receivedDeviceToken("rotated-apns")
        XCTAssertNil(state.beginRemoteRegistration(host: "computer", now: start))
        state.activate(context: ConnectionContext(host: "computer", token: "credential", generation: 1))
        XCTAssertNotNil(state.beginRemoteRegistration(host: "computer", now: start))
    }
    ...
    func testRequestsOncePerEnabledLaunchAndWaitsForCallback() {
        ...
        XCTAssertNil(state.beginRemoteRegistration(host: "computer-a", now: start))
    ...
    func testRemoteRegistrationsArePerHostAndExpire() throws {
        var state = NotificationRegistration()
        state.receivedDeviceToken("test-token-a")
        let request = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-a", now: start))
        XCTAssertNil(state.beginRemoteRegistration(host: "computer-a", now: start))
        XCTAssertFalse(state.usesRemoteNotifications(host: "computer-a", now: start))
        XCTAssertTrue(state.finishRemoteRegistration(request, accepted: true, now: start))
        XCTAssertTrue(state.usesRemoteNotifications(host: "computer-a", now: start))
        XCTAssertFalse(state.usesRemoteNotifications(host: "computer-b", now: start))
        let other = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-b", now: start))
        XCTAssertTrue(state.finishRemoteRegistration(other, accepted: true, now: start))
        XCTAssertTrue(state.usesRemoteNotifications(host: "computer-a", now: start))
        XCTAssertNil(state.beginRemoteRegistration(host: "computer-a", now: start.addingTimeInterval(3599)))
        XCTAssertFalse(state.usesRemoteNotifications(host: "computer-a", now: start.addingTimeInterval(3600)))
        XCTAssertNotNil(state.beginRemoteRegistration(host: "computer-a", now: start.addingTimeInterval(3600)))
    }

    func testChangedTokenBypassesHostTimerAndRejectsStaleCompletion() throws {
        var state = NotificationRegistration()
        state.receivedDeviceToken("test-token-a")
        let accepted = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-a", now: start))
        XCTAssertTrue(state.finishRemoteRegistration(accepted, accepted: true, now: start))
        let stale = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-b", now: start))
        state.receivedDeviceToken("test-token-b")
        XCTAssertFalse(state.isCurrent(stale))
        XCTAssertFalse(state.usesRemoteNotifications(host: "computer-a", now: start))
        let changed = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-a", now: start))
    ...
    func testTokenRotationBackCannotAcceptOldRequest() throws {
        var state = NotificationRegistration()
        state.receivedDeviceToken("test-token-a")
        let stale = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-a", now: start))
        state.receivedDeviceToken("test-token-b")
        state.receivedDeviceToken("test-token-a")
        let current = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-a", now: start))
        XCTAssertFalse(state.isCurrent(stale))
        XCTAssertFalse(state.finishRemoteRegistration(stale, accepted: true, now: start))
    ...
    func testRejectedRenewalEnablesLocalFallbackAndAllowsRetry() throws {
        var state = NotificationRegistration()
        state.receivedDeviceToken("test-token-a")
        let first = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-a", now: start))
        XCTAssertTrue(state.finishRemoteRegistration(first, accepted: true, now: start))
        let later = start.addingTimeInterval(3600)
        let renewal = try XCTUnwrap(state.beginRemoteRegistration(host: "computer-a", now: later))
        ...
        XCTAssertFalse(state.usesRemoteNotifications(host: "computer-a", now: later))
        XCTAssertNotNil(state.beginRemoteRegistration(host: "computer-a", now: later))
    }
}
