...
The bridge requests the session list through the shared management-RPC backend immediately and every two seconds. It skips a polling tick while the previous request is outstanding. For backend installation and gateway configuration, see [README.md](README.md#companion-bridge).

## Snapshots

Every successful poll sends a replacement snapshot with `version: 1`, `type: "snapshot"`, `sessionID`, `epoch`, `sequence`, `status`, and `activity: []`. The status is the selected session's current status. Sequence numbers start at 1 and increase within the connection's epoch, including when status is unchanged.
...
Run `swift test`. Event-stream tests cover subscription validation, authentication and Origin rejection, upgrade-byte handoff, successive polling snapshots with a stable epoch, and independent stream admission. Client tests cover replacement snapshots, sequence gaps and epoch changes, compatibility events, and activity bounds.
